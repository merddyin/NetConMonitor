...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        Encoding = 'utf8'
        ModuleToBuild = 'NetConMonitor'
        ModuleVersion = '0.0.1'
        ModuleWebsite = 'https://github.com/merddyin/NetConMonitor'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) Topher Whitfield. All rights reserved."
        ModuleLicenseURI = 'https://github.com/merddyin/NetConMonitor/LICENSE.md'
        ModuleTags = 'monitor, wmi, network' -split ','
        ModuleAuthor = 'Topher Whitfield'
        ModuleDescription = 'This module assists with the creation of WMI subscriptions to monitor for, and log, new outbound network connections on one or more systems. Subscriptions can be either ephemeral (session only) or persistent, depending on whether the monitor needs to persist through reboots.'
        ...
        # You should NOT enter this key here but rather manually enter it in the NetConMonitor.buildenvironment.json file with: Set-MBBuildEnvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "NetConMonitor.buildenvironment.json"
